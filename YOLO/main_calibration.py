# main_calibration.py
# Calibration entry point: same pipeline as main.py (image capture, local YOLO-based
# component detection, visualization, movement data updates) but with calibration_mode
# hardcoded to True, so running this script directly drives the camera-robot calibration
# flow without needing to flip the flag in main.py.
...
DEFAULT_CONFIG = {
    "calibration_mode": True, # Set to False for normal operation (data update mode)
