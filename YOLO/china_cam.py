...
# The following imports are for non-blocking input, which is not strictly
# necessary for a simple 'press Enter' prompt using input(), but kept
# for completeness if you decide to re-integrate more complex input handling.
# For this script, input() will handle the blocking.
# import termios
# import fcntl
...
def _work_thread_internal(cam, output_dir, capture_done_event):
    ...
    while not _g_bExit_internal:
        # Wait for the signal to capture, with a small timeout to check exit flag
        ...
        if _capture_event_internal.is_set():
            print("Attempting to get image buffer...")
            ret = cam.MV_CC_GetImageBuffer(stOutFrame, 5000) # 5 second timeout
            if ret == 0:
                ...
                if is_bayer_format:
                    ...
                    if ret_convert == 0:
                        ...
                        print(f"Failed to convert pixel format! Return code: [0x{ret_convert:x}]", file=sys.stderr)
                ...
                stSaveParam.enImageType = MV_Image_Jpeg # Save as JPEG
                stSaveParam.nQuality = 80 # JPEG quality
                ...
                if ret_save == 0:
                    ...
                    print(f"Failed to save image! Return code: [0x{ret_save:x}]", file=sys.stderr)
                    _captured_image_path_internal = None

                cam.MV_CC_FreeImageBuffer(stOutFrame) # Free the image buffer
            else:
                print(f"No data [0x{ret:x}] or failed to get image buffer within timeout.", file=sys.stderr)
                _captured_image_path_internal = None

            _capture_event_internal.clear() # Reset the event for the next capture
            capture_done_event.set() # Signal that capture/save attempt is done

    print("Internal work thread exiting.")
...
def capture_and_save_single_image(output_dir='.', camera_index=0):
    ...
    # Event to signal when the capture/save process in the work thread is complete
    ...
    exposure_time_us = 60000 # Example exposure time in microseconds
    ...
    # Configure GigE device packet size if applicable
    ...
    # data_buf is not explicitly used here but often allocated for image acquisition
    # In this specific setup, the image buffer is managed by MV_CC_GetImageBuffer
    # and freed by MV_CC_FreeImageBuffer.
    data_buf = (c_ubyte * nPayloadSize)() # Keep it for consistency if needed by SDK internals

    try:
        # Start the internal work thread for capturing and saving
        hThreadHandle = threading.Thread(target=_work_thread_internal, args=(cam, output_dir, capture_done_event))
        hThreadHandle.daemon = True # Allow the main program to exit even if this thread is running
        hThreadHandle.start()
    ...
    print ("Camera ready. Signaling internal thread to capture single image...")
    _capture_event_internal.set() # Signal the internal thread to capture one image

    # Wait for the capture/save process to complete, with a timeout
    if not capture_done_event.wait(timeout=15): # 15 seconds timeout for capture and save
    ...
    # Signal the internal thread to exit and wait for it to finish
    _g_bExit_internal = True
    _capture_event_internal.set() # Ensure the thread wakes up to see the exit flag
    hThreadHandle.join(timeout=2) # Give the thread a moment to clean up

    # Stop grabbing, close device, and destroy handle
    ...
    MvCamera.MV_CC_Finalize() # Finalize SDK
    print("Camera resources cleaned up.")

    del data_buf # Clean up allocated buffer
...
if __name__ == "__main__":
    ...
    print(f"Images will be saved in: {script_dir}")

    while True:
        user_input = input("Press Enter to take a photo, or type 'q' to quit: ").strip().lower()
        if user_input == 'q' or user_input == 'quit':
            print("Exiting camera capture program.")
            break
        elif user_input == '':
            print("Taking photo...")
            captured_path = capture_and_save_single_image(output_dir=script_dir, camera_index=0)
            if captured_path:
                print(f"Photo saved to: {captured_path}")
            else:
                print("Failed to capture or save photo.")
